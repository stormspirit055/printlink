...
function hall() {
  return shell(
    `<div class="page-head"><div><div class="eyebrow">PUBLIC JOB BOARD / 实时更新</div><h1>制造需求大厅</h1><p>所有需求均经人工审核，打印方自主判断并报价。</p></div><button class="btn" data-action="publish">发布打印需求</button></div><div class="toolbar"><input class="input search" id="search" placeholder="搜索需求、材料或城市"><select class="select" id="matFilter" style="width:160px"><option value="">全部材料</option>${state.config.materials.map((m) => `<option>${m.code}</option>`).join("")}</select><select class="select" style="width:150px"><option>最新发布</option><option>预算从高到低</option></select></div><div class="grid" id="demandGrid">${state.demands.length ? state.demands.map(demandCard).join("") : '<div class="empty">暂无开放需求</div>'}</div>`,
  );
}
async function workspace() {
  const mine = await api(`/api/demands?mine=${state.user.id}`);
  return shell(
    `<div class="page-head"><div><div class="eyebrow">PERSONAL CONSOLE</div><h1>${esc(state.user.nickname)}的工作台</h1><p>管理你发布的需求和收到的报价。</p></div><button class="btn" data-action="publish">发布新需求</button></div><div class="kpis"><div class="kpi"><span class="meta">发布需求</span><strong>${mine.length}</strong></div><div class="kpi"><span class="meta">进行中订单</span><strong>${state.orders.filter((o) => !["completed"].includes(o.status)).length}</strong></div><div class="kpi"><span class="meta">已完成交易</span><strong>${state.orders.filter((o) => o.status === "completed").length}</strong></div><div class="kpi"><span class="meta">打印节点</span><strong>${state.printers.length}</strong></div></div><div class="table-wrap"><table class="table"><thead><tr><th>需求编号</th><th>标题</th><th>状态</th><th>预算</th><th>报价</th><th>发布时间</th></tr></thead><tbody>${mine.map((d) => `<tr data-demand="${d.id}"><td>REQ-${String(d.id).padStart(5, "0")}</td><td>${esc(d.title)}</td><td><span class="tag ${d.status === "rejected" ? "" : "status"}">${statusMap[d.status]}</span></td><td>${fmt(d.budget)}</td><td>${d.quote_count || 0}</td><td>${date(d.created_at)}</td></tr>`).join("")}</tbody></table></div>`,
  );
}
function orders() {
  return shell(
...
function printers() {
  return shell(
    `<div class="page-head"><div><div class="eyebrow">MAKER NODES</div><h1>打印设备</h1><p>录入真实硬件能力，接单报价时选择执行设备。</p></div><button class="btn" data-action="addPrinter">登记打印机</button></div><div class="grid">${state.printers.map((p) => `<div class="card"><div class="model-window"><div class="model-glyph"></div></div><div class="card-body"><div class="card-top"><span class="tag status">NODE ONLINE</span><span class="meta">${esc(p.technology)}</span></div><h3>${esc(p.name)}</h3><p class="desc">${esc(p.model)} · ${esc(p.description || "未填写设备说明")}</p><div class="specs"><span class="tag">${p.max_x}×${p.max_y}×${p.max_z} mm</span><span class="tag">${p.color_mode === "multi" ? `最多 ${p.max_colors} 色` : "单色"}</span><span class="tag">${p.enclosed ? "封闭仓" : "开放式"}</span></div></div></div>`).join("") || '<div class="empty">还没有登记打印设备</div>'}</div>`,
  );
}
